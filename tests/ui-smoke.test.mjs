...
test('紧凑界面、歌曲选择、分类返回、收藏、文件夹布局和歌单弹窗可正常工作', async () => {
  ...
  assert.match(styles, /\.player-lyric \{[^}]*text-align: center/);
  assert.match(styles, /\.player-favorite\.active \{ color: var\(--md-error, #d32f2f\);/);
  assert.match(script, /class PlayerBridge \{/);
  assert.match(script, /return playerBridge\.setQueue\(ids, index\);/);
  assert.match(script, /async function loadLyric\(id\) \{/);
  assert.match(script, /\/api\/v1\/songs\/\$\{id\}\/lyric/);
  assert.match(script, /async function togglePlayerFavorite\(\) \{/);
  assert.match(script, /async function playSingleSong\(id\) \{/);
  assert.match(styles, /\.sidebar \{ position: fixed;/);
  assert.match(styles, /\.main-panel \{ grid-column: 2;/);
  assert.match(styles, /\.player-bar\.collapsed > \*:not\(#playerCover\) \{ display: none !important; \}/);
  assert.match(script, /function setPlayerCollapsed\(collapsed\) \{/);
  assert.match(html, /id="playerCollapse"/);
  assert.match(styles, /html \{ overflow-x: hidden; \}/);
  assert.doesNotMatch(styles, /body \{[^}]*overflow-x: hidden/);
  assert.match(styles, /\.player-bar\.collapsed \{ left: auto; right: 12px;/);
  ...
  window.scrollTo = () => {};
  const playerState = { queue: [], current_index: -1, current_song: null, is_playing: false, play_mode: 'order', volume: 70, position: 0 };
  const playerListeners = [];
  const emitPlayerState = () => playerListeners.forEach((listener) => listener(structuredClone(playerState)));
  const setQueueCalls = [];
  const playIdCalls = [];
  const addToQueueCalls = [];
  const player = {
    getState: async () => structuredClone(playerState),
    onStateChange: (listener) => playerListeners.push(listener),
    setQueue: async (ids, opts) => {
      setQueueCalls.push({ ids, opts });
      playerState.queue = songs.filter((song) => ids.includes(song.id));
      playerState.current_index = playerState.queue.length
        ? Math.min(Number(opts?.startIndex || 0), playerState.queue.length - 1) : -1;
      playerState.current_song = playerState.queue[playerState.current_index] || null;
      playerState.is_playing = playerState.current_index >= 0;
      emitPlayerState();
    },
    addToQueue: async (ids) => {
      addToQueueCalls.push(ids);
      const additions = songs.filter((song) => ids.includes(song.id)
        && !playerState.queue.some((queued) => queued.id === song.id));
      playerState.queue = [...playerState.queue, ...additions];
      emitPlayerState();
    },
    play: async (id) => {
      playIdCalls.push(id);
      const found = playerState.queue.findIndex((song) => song.id === id);
      if (found >= 0) playerState.current_index = found;
      playerState.current_song = playerState.queue[playerState.current_index] || null;
      playerState.is_playing = playerState.current_index >= 0;
      emitPlayerState();
    },
    togglePlay: async () => { playerState.is_playing = !playerState.is_playing; emitPlayerState(); },
    next: async () => {},
    prev: async () => {},
    seek: async () => {},
    pause: async () => { playerState.is_playing = false; emitPlayerState(); },
    setVolume: async () => {},
    setPlayMode: async () => {},
  };
  const posts = [];
  ...
  window.SongloftPlugin = {
    apiGet: async (path) => {
      gets.push(path);
      if (path.startsWith('/api/capabilities')) return { hostVersion: '2.11.0', language: true, style: true };
    ...
    host: {
      isAvailable: () => true,
      getInfo: async () => ({ version: '2.11.0', platform: 'test', capabilities: ['player'] }),
    },
    player,
  };
  ...
  assert.ok(window.document.querySelector('[data-sort="favorite"] .favorite-header-icon .heart-icon'));
  ...
  await waitFor(() => setQueueCalls.length === 1);
  assert.deepEqual(setQueueCalls[0].ids, [1, 2]);
  assert.equal(setQueueCalls[0].opts.startIndex, 0);
  ...
  assert.ok(!window.document.querySelector('#playerBar').classList.contains('hidden'));
  ...
  window.document.querySelector('#playerTitle').click();
  assert.ok(window.document.querySelector('#queuePanel').classList.contains('hidden'));
  window.document.querySelector('#playerQueueButton').click();
  assert.ok(!window.document.querySelector('#queuePanel').classList.contains('hidden'));
  window.document.querySelector('#queueClose').click();
  assert.ok(window.document.querySelector('#queuePanel').classList.contains('hidden'));
  const playerFavorite = window.document.querySelector('#playerFavorite');
  assert.ok(playerFavorite.classList.contains('active'));
  playerFavorite.click();
  await waitFor(() => !playerFavorite.classList.contains('active'));
  assert.deepEqual(JSON.parse(JSON.stringify(posts.filter((item) => item.path === '/api/favorites/set').pop().body)), { id: 1, favorite: false });
  playerFavorite.click();
  await waitFor(() => playerFavorite.classList.contains('active'));
  assert.deepEqual(JSON.parse(JSON.stringify(posts.filter((item) => item.path === '/api/favorites/set').pop().body)), { id: 1, favorite: true });
  const playerBar = window.document.querySelector('#playerBar');
  const playerCollapse = window.document.querySelector('#playerCollapse');
  playerCollapse.click();
  assert.ok(playerBar.classList.contains('collapsed'));
  assert.equal(playerCollapse.getAttribute('aria-expanded'), 'false');
  window.document.querySelector('#playerCover').click();
  assert.ok(!playerBar.classList.contains('collapsed'));
  assert.equal(playerCollapse.getAttribute('aria-expanded'), 'true');
  ...
  assert.equal(setQueueCalls.length, 1);
  ...
  await waitFor(() => playIdCalls.includes(1));
  assert.equal(setQueueCalls.length, 1);
  assert.equal(addToQueueCalls.length, 0);
  ...
  await waitFor(() => setQueueCalls.length === 2);
  ...
  await waitFor(() => window.document.querySelector('#removePlaylistDialog').open);
  window.document.querySelector('#confirmRemovePlaylistButton').click();
